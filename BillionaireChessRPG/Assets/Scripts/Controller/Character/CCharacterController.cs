...
namespace BillianaireChessRPG {
	...
	public partial class CCharacterController : CObjectController {
		...
		public override void MoveToTargetBlock (float dt)
		{
			base.MoveToTargetBlock (dt);
			var currentBlock = this.GetCurrentBlock () as CBlockController;
			var targetBlock = this.GetTargetBlock () as CBlockController;
			var nextBlock = m_MapManager.GetBlockPath (0, currentBlock, targetBlock);
			var distance = (nextBlock.GetPosition () - this.GetPosition ()).sqrMagnitude;
			if (distance >= this.GetDistanceToTarget() * this.GetDistanceToTarget()){
				this.MoveToTarget (nextBlock.GetPosition (), dt);
			} else {
