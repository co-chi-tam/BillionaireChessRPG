...
namespace BillianaireChessRPG {
	...
	public class CJumperController : CMonsterController {
		...
		#region Implementation Monobahaviour
		...
		{
			var target = this.GetTargetBlock () as CBlockController;
			JumpToPosition (target.GetPosition ());
		}
		...
		public virtual void JumpToPositions(params Vector3[] positions) {
			if (m_Jumping == false) {
				m_Jumping = true;	
				JumpToPositions (OnClientJumpComplete, () => {
					m_Jumping = false;	
