...
namespace BillianaireChessRPG {
	public class CMovableComponent : CComponent {
		...
		public CMovableComponent (IMovable movable) : base()
		{
			m_Angle 			= 0f;
			m_SpeedThreshold 	= 1f;
			m_Target 			= movable;
		...
		public virtual void LookForwardToTarget(Vector3 value) {
			if (currentTransform != null) {
				...
				var forward = currentTransform.forward;
				m_Angle = Mathf.Atan2 (m_Direction.x, m_Direction.z) * Mathf.Rad2Deg;
//				currentTransform.rotation = Quaternion.Lerp (currentTransform.rotation, Quaternion.AngleAxis (m_Angle, Vector3.up), 0.25f);
				currentTransform.rotation = Quaternion.AngleAxis (m_Angle, Vector3.up);
			}
		}
